...
async function CreatePoolHandler(req, res) {
  ...
  const { token_id, name, description } = req.body;
  ...
  if (poolExists) {
    return Response.unprocessibleEntity(res, {
      error: `Pool with token_id: ${token_id} already exists`,
    });
  }
...
export default prepare(withRegisteredAccount)(CreatePoolHandler);
