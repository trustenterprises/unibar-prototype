...
async function DepositTokenPoolHandler(req, res) {
  ...
  if (!token) {
    return Response.unprocessibleEntity(res, {
      error: `Token with token_id: ${token_id} does not exist`,
    });
  }
  ...
  if (!pool) {
    return Response.unprocessibleEntity(res, {
      error: `Pool with token_id: ${token_id} does not exist`,
    });
  }
  ...
  if (currentRplPool) {
    return Response.unprocessibleEntity(res, {
      error: `Reward Liquidity Pool for user: ${authorisationAccount.hedera_id} for token ${token_id} already exists`,
    });
  }
  ...
  const proposedAmount =
    parseFloat(amount) * 10 ** userTokenHolding.token.decimals;
  ...
  if (proposedAmount > tokenHodlStack) {
    return Response.unprocessibleEntity(res, {
      error: `You cannot deposit more tokens then you own into the pool`,
    });
  }
  ...
  const { proxyLiquidityPool, account } =
    await service.createNewRewardPoolAccount();
...
export default prepare(withRegisteredAccount)(DepositTokenPoolHandler);
