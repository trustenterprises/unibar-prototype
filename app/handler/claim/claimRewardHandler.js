...
async function ClaimRewardTransactionHandler(req, res) {
  ...
  if (!holding) {
    return Response.unprocessibleEntity(res, {
      error: `Unable to see ${token_id} in your holdings`,
    });
  }
  ...
  if (holding.token.spec_ref !== LpRef) {
    return Response.unprocessibleEntity(res, {
      error: `You can only claim rewards with Liquity Reward tokens with the reference "${LpRef}"`,
    });
  }
  ...
  if (proposedAmount > holding.amount) {
    return Response.unprocessibleEntity(res, {
      error: `You cannot withdraw more receipt tokens then you own`,
    });
  }

  const removePercentageOfPool = Math.floor(
    (proposedAmount / holding.amount) * 100
  );

  if (removePercentageOfPool === 0) {
    return Response.unprocessibleEntity(res, {
      error: `That amount is too low to claim back rewards`,
    });
  }
