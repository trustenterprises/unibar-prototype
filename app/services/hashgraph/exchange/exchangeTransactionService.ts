...
class ExchangeTransactionService {
  ...
  constructor({ hashgraphClient, authorisationAccount, pool, token, amount }) {
  ...
  async rewardPoolDistribution() {
    ...
    const total = rewardPools.reduce(
      (acc, pool) => acc + parseInt(pool.amount),
      0
    );
    ...
    const rewardDistributions = rewardPools.map(async pool => {
      const account = await this.extractAccountWithPrivateKey(
        pool.escrowTreasuryAccount.account
      );

      return {
        total: Math.floor(
          (parseInt(pool.amount) / total) * parseInt(this.amount)
        ),
