...
const cssState = {
  selected:
    "py-6 border-b border-white pb-4 cursor-pointer font-bold xl:flex items-center text-sm text-white tracking-normal  mr-10 ",
  default:
    "py-6 cursor-pointer font-bold xl:flex items-center text-sm text-gray-300 mr-10 tracking-normal",
};
  ...
  return (
    <ul className="flex items-start justify-center h-full">
      ...
      <li className="mr-8 cursor-pointer h-full flex items-center">
        ...
        />
        <img
          src="https://landen.imgix.net/avz5ymmxk75u/assets/rueradx7.svg?w=104"
          alt="logo"
        />
      </li>
      <li
        data-id={sections.project.key}
        onClick={selectNavigationSection}
        className={getDisplayState(sections.project.key)}>
        <a>{sections.project.title}</a>
      </li>
      <li
        data-id={sections.bridge.key}
        onClick={selectNavigationSection}
