...
function TokenSummary() {
  ...
  const normalHoldings = holdings.filter(
    holding =>
      holding.token.spec_ref !==
      Specification.UnibarLiquidityProviderReceipt.reference
  );

  const lpHoldings = holdings.filter(
    holding =>
      holding.token.spec_ref ===
      Specification.UnibarLiquidityProviderReceipt.reference
  );
  ...
  const holdingsPercent =
    Math.round((normalHoldings.length / totalTokens) * 100) | 0;
  const lpPercent = Math.round((lpHoldings.length / totalTokens) * 100) | 0;
  const mintedPercent =
    Math.round((mintedTokens.length / totalTokens) * 100) | 0;
