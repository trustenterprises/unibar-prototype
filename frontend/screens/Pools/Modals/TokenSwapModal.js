...
function TokenSwapModal({ closeModal, currentPool, holdings }) {
  ...
  const holdingsWithBalance = holdings.filter(holding => {
    const ensureFungible =
      holding.token.spec_ref === Specification.Fungible.reference;
    const ensureNotPooled =
      holding.token.token_id !== currentPool.token.token_id;
  ...
  const lowBalance =
    currentTokenInput >=
    parseInt(currentToken.amount) / 10 ** currentToken.token?.decimals;
  ...
  const onSwapToken = () => {
    ...
    toast.info(
      `Attempting to swap ${currentToken.token.symbol} tokens for ${currentTokenInput} ${currentPool.token.symbol}`
    );
    ...
    exchangeSwapTokens(payload, onFail).then(pool => {
      toast.success(
        `Swapped ${currentTokenInput} ${currentToken.token.symbol} tokens for  ${currentTokenInput} ${currentPool.token.symbol}`
      );
  ...
  const switchSelectedToken = event => {
    ...
    const holding = holdingsWithBalance.filter(
      holding => holding.token.id === parseInt(tokenId)
    );
    ...
    <div className="flex items-center justify-center py-8 px-4">
      <div className="md:w-80 w-full  rounded shadow-lg p-6  dark:bg-gray-800 bg-white">
        <h1 className=" dark:text-gray-100 text-gray-800 font-bold text-lg">
          Swap your tokens 1:1 for {currentPool.token.symbol}
        </h1>
        <h3 className=" dark:text-gray-00 text-gray-400 font-bold text-sm">
          Current {currentPool.token.symbol} liquidity {tokensInPool} tokens
        </h3>
        ...
        <div>
          <div className="relative flex items-center justify-center ">
            <select
              ...
              <option
                key={0}
                className="text-base font-medium  dark:text-gray-100 text-gray-800">
                Select a token
              </option>
              {holdingsWithBalance.map(holding => (
                <option
                  ...
                  {holding.amount / 10 ** holding.token.decimals})
                </option>
              ))}
            </select>
          ...
          <p className=" text-xs font-semibold pb-4 leading-4 text-indigo-700 uppercase">
            AMOUNT
          </p>
        ...
        {!lessThenLiquidity ||
          (lowBalance && (
            <p className="pt-4 text-xs font-semibold leading-4 text-red-500 uppercase">
              UNABLE TO SWAP ASSETS
            </p>
          ))}

        {canConfirm && (
          <p className="pt-4 text-xs font-semibold leading-4 text-indigo-500 uppercase">
            SWAP {currentTokenInput} {currentToken.token.symbol} for{" "}
            {currentTokenInput} {currentPool.token.symbol}
          </p>
        )}
        ...
        <div className="pt-4 flex items-center justify-between  ">
          ...
          <div className="pl-2 w-full">
            <button
              onClick={onSwapToken}
              disabled={!canConfirm}
              className="disabled:opacity-50 py-3.5 w-full   text-white focus:outline-none hover:opacity-90 rounded border text-sm font-semibold border-indigo-700 bg-indigo-700">
              Confirm
            </button>
          </div>
        </div>
