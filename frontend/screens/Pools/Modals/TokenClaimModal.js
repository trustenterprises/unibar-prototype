...
function TokenClaimModal({ closeModal, currentPool, receiptToken }) {
  ...
  const maxTokens =
    parseInt(receiptToken.amount) / 10 ** receiptToken.token.decimals;
  ...
  const onClaimedToken = () => {
    ...
    toast.info(
      `Attempting to claim tokens ${currentTokenInput} from ${receiptToken.token.symbol}`
    );
    ...
    claimTokenFromPool(payload, onFail).then(pool => {
      ...
      toast.success(
        `Claimed tokens ${currentTokenInput} from ${receiptToken.token.symbol}`
      );
    ...
    <div className="flex items-center justify-center py-8 px-4">
      <div className="md:w-80 w-full  rounded shadow-lg p-6  dark:bg-gray-800 bg-white">
        <h1 className=" dark:text-gray-100 text-gray-800 font-bold text-lg">
          Claim your {currentPool.token.symbol} and swapped assets using{" "}
          {receiptToken.token.symbol}
        </h1>
        <div>
          <p className=" text-xs font-semibold mt-6 pb-4 leading-4 text-indigo-700 uppercase">
            Number of tokens (max {maxTokens})
          </p>
