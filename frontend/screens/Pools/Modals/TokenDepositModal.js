...
function TokenDepositModal({ closeModal, currentPool, maxTokens = 10 }) {
  ...
  const onDepositToken = () => {
    ...
    toast.info(
      `Attempting to deposit ${currentTokenInput} ${currentPool.token.symbol} tokens into ${currentPool.name}`
    );
    ...
    depositTokenIntoPool(payload, onFail).then(pool => {
      ...
      toast.success(
        `Deposited ${currentTokenInput} ${currentPool.token.symbol} tokens into ${currentPool.name}`
      );
    ...
    <div className="flex items-center justify-center py-8 px-4">
      <div className="md:w-80 w-full  rounded shadow-lg p-6  dark:bg-gray-800 bg-white">
        <h1 className=" dark:text-gray-100 text-gray-800 font-bold text-lg">
          Deposit {currentPool.token.symbol} tokens into {currentPool.name}
        </h1>
        <div>
