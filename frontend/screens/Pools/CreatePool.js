...
function CreatePool() {
  ...
  const poolTokenSet = pools.length
    ? new Set(pools.map(pool => pool.token.token_id))
    : new Set();
  const holdingsWithoutPooled = account?.holdings?.filter(
    holding => !poolTokenSet.has(holding.token.token_id)
  );
          ...
          <div className="xl:w-full border-b border-gray-300 py-5">
            <div className="flex items-center w-11/12 mx-auto">
              <p className="text-lg text-gray-800 font-bold">
                {account
                  ? "Create new pool"
                  : "Please authenticate to create a pool"}
              </p>
            </div>
          </div>
                      ...
                      <div className="relative">
                        <div
                          ...
                          <p className="pl-3 py-3 text-white text-sm leading-3 tracking-normal font-normal">
                            {tokenSelected.token?.symbol || "Select a token"}
                          </p>
                        ...
                        {show && (
                          <ul className="visible transition duration-300 opacity-100 ease-out bg-white shadow rounded mt-2 py-1 w-48 absolute">
