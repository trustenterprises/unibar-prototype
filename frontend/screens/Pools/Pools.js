...
function Pools() {
  ...
  const tokenHoldings = new Set(
    account?.holdings?.map(holding => holding.token.token_id)
  );
  const tokenSymbols = new Set(
    account?.holdings?.map(holding => holding.token.symbol)
  );
  ...
  if (showDeposit) {
    const holding = account.holdings.filter(
      holding => holding.token.token_id === currentPool.token.token_id
    );
  ...
  if (showClaim) {
    ...
    const receipt = account.holdings.filter(
      holding => holding.token.symbol === `${token.token.symbol}:LP:RECEIPT`
    );
          ...
          <div className="rounded shadow border-2">
            <div className="bg-white dark:bg-gray-800  rounded py-6 px-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm leading-none text-gray-500 dark:text-gray-200">
                    ({pool.token.symbol})
                  </p>
                  <p className="text-4xl font-semibold leading-9 pt-4 text-gray-800 dark:text-gray-100">
                    {parseFloat(
                      (pool.amount / 10 ** pool.token.decimals).toFixed(3)
                    )}
                  </p>
                </div>
            ...
            <div className="items-center px-4 pb-4 flex flex-wrap">
              ...
              {parseInt(pool.amount) > 0 &&
                tokenSymbols.has(`${pool.token.symbol}:LP:RECEIPT`) && (
                  <button
                    className="mx-2 my-2 bg-red-100 transition duration-150 ease-in-out hover:bg-red-200 rounded border border-red-300 text-red-600 px-5 py-1 text-xs"
              ...
              {parseInt(pool.amount) === 0 &&
                tokenHoldings.has(pool.token.token_id) && (
                  <button
                    className="mx-2 my-2 bg-blue-200 transition duration-150 ease-in-out focus:outline-none hover:bg-blue-300 rounded text-blue-700 px-5 py-1 text-xs"
